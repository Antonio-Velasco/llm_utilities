...
def process_pdf(uploaded_file, page_range, clean_document):
    ...
    if clean_document:
        hfooter = ft.reduce(set.intersection,
                            (set(page) for page in pages[:10])
                            )
        pages = [[line for line in page if line not in hfooter]
                 for page in pages]
...
uploaded_file = st.file_uploader(
    "Upload a document to summarize, 10k to 100k tokens works best!",
    type=['txt', 'pdf']
    )
...
if is_pdf(uploaded_file):
    with st.expander("For this file, how should it be processed"):
        if max_pages > 0:
            page_range = st.slider("Select page range to summarize",
                                   value=[1, max_pages],
                                   min_value=1, max_value=max_pages
                                   )
...
"""
number_of_paragraphs = st.number_input(label="Number of paragraphs in summary",
...
if st.button("Summarize file", type="primary"):
    with st.spinner("SummarAIzing hard..."):
        ...
        summary_file, text = get_text_and_summary_file(uploaded_file,
                                                       url,
                                                       page_range,
                                                       clean_document
                                                       )
        ...
        with st.expander("Cost and file download", expanded=True):
            ...
            if is_pdf(uploaded_file):
                btn = st.download_button("Download source file plain text",
                                         text,
                                         file_name=f"{uploaded_file.name}.txt"
                                         )

            st.download_button("Download summarized text",
                               summarized_text,
                               file_name=f"{summary_file}-summarize.md"
                               )
